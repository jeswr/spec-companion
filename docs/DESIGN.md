...
5. `agentic-solid-conformance` clause→statement-id migration (additive). **Begun**
   (2026-07-07): the `a2a-rdf` and `agent-authz-credential` suites now carry companion
   `statements` ids + a `statementIndex`/`statementCompanion` in their manifests,
   derived from and kept an exact inverse of each companion's `spec:testCase` links;
   `odrl-delegation` awaits its companion. The migration surfaced one companion
   coverage gap (`agent-authz-credential-spec` does not pin
   `vec:identity-composition-wrong-root` to `AAC-VER-5` though its two siblings map
   there) — a companion errata candidate.
6. Phase-2 central catalog (VoID/DCAT list of companion URLs). **Delivered**
   (2026-07-07): [`catalog/`](../catalog/) — a DCAT `dcat:Catalog` of all 11 landed
   companions (by reference, never by copy), generated by `tools/catalog.mjs` and
   gate-checked (`catalog:check` in the base gate). The optional
   `<link rel="alternate">` per-spec discovery hint remains maintainer-steer (it
   touches spec HTML) and is not part of this catalog.
